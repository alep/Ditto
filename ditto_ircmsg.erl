...
%% Constants (from RFC 2812) -- Taken from Twisted source
'RPL_WELCOME'() -> "001".
'RPL_YOURHOST'() -> "002".
'RPL_CREATED'() -> "003".
'RPL_MYINFO'() -> "004".
...
symbolic_to_numeric() ->
    dict:from_list(symbolic_to_numeric_()).

numeric_to_symbolic() ->
    dict:from_list(lists:map(fun({X, Y}) -> {Y, X} end,
                             symbolic_to_numeric_())).


get_command_from_num(Command) ->
    D = numeric_to_symbolic(),
    case dict:find(Command, D) of
        {ok, Value} ->
            Value;
        error ->
            Command
